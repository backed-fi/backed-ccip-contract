...
task(
  ...
  .setAction(
    async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
      ...
      console.log(
        `ℹ️  Attempting to register token ${token} in BackedCCIPReceiver on the ${hre.network.name} blockchain using tokenId: ${tokenId} provided as constructor argument`
      );
      spinner.start();
      ...
      await contract.registerToken(token, tokenId);
