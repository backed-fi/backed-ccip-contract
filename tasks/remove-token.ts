...
task(
  `remove-token`,
  `Removes token from BackedCCIPReceiver smart contract`
)
  ...
  .setAction(
    async (taskArguments: TaskArguments, hre: HardhatRuntimeEnvironment) => {
      ...
      console.log(
        `ℹ️  Attempting to remove token: ${token} from BackedCCIPReceiver on the ${hre.network.name} blockchain`
      );
      spinner.start();
      ...
      await contract.removeToken(token);

      spinner.stop();
      console.log(
        `✅ Removed token: ${token} from BackedReceiverCCIP on ${hre.network.name} blockchain`
      );
    }
  );
